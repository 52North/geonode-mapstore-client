...
import {
  DEFAULT_FORMAT_WMS,
  getSupportedFormat,
} from "@mapstore/framework/api/WMS";
...
function WMSLayerSettings({
  ...
  const [settingsSections, setSettingsSections] = useLocalStorage(
    "settings-section",
    {
      general: true,
      visibility: false,
  ...
  function areLegendOptionsValid() {
    return (
      isLegendOptionValid("legendWidth") !== "error" &&
  ...
  return (
    <>
      ...
      >
        <Checkbox
          key="layerStyleSelect"
          checked={userLayerStyleSelect}
          onChange={(event) =>
            onChange({ userLayerStyleSelect: event.target.checked })
          }
        >
          {/*  <Message msgId="layerProperties.userLayerStyleSelect" /> */}
          <div>User Layersyle waehlen lassen </div>
        </Checkbox>

        {styleSelectorComponent}
        <FormGroup validationState={isLegendOptionValid("legendWidth")}>
          <ControlLabel>
            <Message msgId="gnviewer.legendWidth" />
          </ControlLabel>
          <IntlNumberFormControl
            ...
            onChange={(value) =>
              onChange({
                legendOptions: {
                  ...legendOptions,
                  legendWidth: value && clamp(Math.round(value), 0, 1000),
                },
              })
            }
        ...
        <FormGroup validationState={isLegendOptionValid("legendHeight")}>
          <ControlLabel>
            <Message msgId="gnviewer.legendHeight" />
          </ControlLabel>
          <IntlNumberFormControl
            ...
            onChange={(value) =>
              onChange({
                legendOptions: {
                  ...legendOptions,
                  legendHeight: value && clamp(Math.round(value), 0, 1000),
                },
              })
            }
            onKeyPress={(event) => event.key === "-" && event.preventDefault()}
        ...
        <FormGroup>
          <ControlLabel>
            <Message msgId="gnviewer.legendPreview" />
          </ControlLabel>
          <div style={{ width: "100%", overflow: "auto" }}>
            <LegendImage
              ...
              layer={node}
              legendHeight={
                (areLegendOptionsValid() && legendOptions.legendHeight) ||
                undefined
              }
              legendWidth={
                (areLegendOptionsValid() && legendOptions.legendWidth) ||
                undefined
              }
              language={
                isLocalizedLayerStylesEnabled
      ...
      >
        <FormGroup>
          <ControlLabel>
            <Message msgId="layerProperties.format.title" />
          </ControlLabel>
        ...
        <FormGroup>
          <Checkbox
            ...
            checked={transparent}
            onChange={(event) =>
              onChange({ transparent: event.target.checked })
            }
          >
          ...
          {isLocalizedLayerStylesEnabled ? (
            <Checkbox
              ...
              onChange={(event) =>
                onChange({ localizedLayerStyles: event.target.checked })
              }
            >
              ...
              <InfoPopover
                text={
                  <Message msgId="layerProperties.enableLocalizedLayerStyles.tooltip" />
                }
              />
            </Checkbox>
          ) : null}
